...
def load_candidates_from_db():
    ...
    try:
        ...
        # --- Format Timestamp ---
        df["recorded_at"] = pd.to_datetime(df["recorded_at"]).dt.strftime(
            "%Y-%m-%d %H:%M"
        )

        # --- Format Numeric Factors ---
        if "momentum_12m" in df.columns:
            df["momentum_12m"] = df["momentum_12m"].apply(
                lambda x: f"{x*100:.2f}%" if pd.notna(x) else "N/A"
            )
        if "debt_to_equity" in df.columns:
            df["debt_to_equity"] = df["debt_to_equity"].apply(
                lambda x: f"{x:.2f}" if pd.notna(x) else "N/A"
            )
        if "return_on_equity" in df.columns:
            df["return_on_equity"] = df["return_on_equity"].apply(
                lambda x: f"{x*100:.2f}%" if pd.notna(x) else "N/A"
            )
        if "annualized_volatility" in df.columns:
            df["annualized_volatility"] = df["annualized_volatility"].apply(
                lambda x: f"{x*100:.2f}%" if pd.notna(x) else "N/A"
            )
        # --- NEW: Format Google Trends Score ---
        if "google_trends_score" in df.columns:
            df["google_trends_score"] = df["google_trends_score"].apply(
                lambda x: f"{x*100:.2f}%" if pd.notna(x) else "N/A"
            )
                ...
                dbc.Tab(
                    ...
                    children=[
                        dbc.Container(
                            [
                                html.H3(
                                    "Principal Component Analysis", className="mt-3"
                                ),
                                ...
                                dbc.Row(
                                    [
                                        dbc.Col(
                                            dcc.Graph(id="explained-variance"), width=6
                                        ),
...
@app.callback(Output("candidates-table", "data"), Input("fit-score-slider", "value"))
