...
def run_crossover_backtest(short_window: int, long_window: int, start_date: str = None, end_date: str = None):
    """
    ...
    """
    test_period = f"{start_date} to {end_date}" if start_date and end_date else "full period"
    print(f"\n\n--- Starting Back-test for {short_window}/{long_window} Crossover ({test_period}) ---")

    # 1. Load top assets from the database
    try:
        con = duckdb.connect(database=DB_FILE, read_only=True)
        top_assets_df = con.execute(f"SELECT symbol FROM candidates ORDER BY fit_score DESC LIMIT {NUM_ASSETS_TO_TEST}").fetchdf()
        con.close()
    ...
    price_data = yf.download(symbols, start=start_date, end=end_date, period=period, auto_adjust=True)['Close']
    if price_data.empty:
